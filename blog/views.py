...
def serialize_post(post):
    return {
        ...
        'slug': post.slug,
        'tags': [serialize_tag_for_posts(tag) for tag in post.tags.all()],
...
def serialize_tag_for_posts(tag):
    return {
        'title': tag.title,
...
def tag_filter(request, tag_title):
    try:
        ...
        return HttpResponseNotFound('<h1>Тэг не найден</h1>')
    ...
    related_posts = tag.posts.prefetch_related('author', 'tags').annotate(
        comments_count=Count('comments')
    )[:20]
